#Automated testing for HPT
library(psychTestR)
library(testthat)

dir <- system.file("HPT", package = "HPT", mustWork = TRUE)
...
print(j)
HPT_SEM[j] <<- results[["HPT"]][["ability_sem"]]
HPT_ability[j] <<- results[["HPT"]][["ability"]]

print(paste("Standard error of measurement of HPT", HPT_SEM[j]))
app$stop()
